...
public class UserTest extends AbstractTest {
    ...
    public void register() {
        ...
        if(StatusCode.NO_USER==userService.loginUser(parentLoginDTO)){
            assertEquals(StatusCode.SUCCESS,userService.registerUser(parentsRegisterRequestDTO));
        }
        if(StatusCode.NO_USER==userService.loginUser(childLoginDto)){
    ...
    @Order(2)
    public void login() {
        assertEquals(StatusCode.SUCCESS,userService.loginUser(parentLoginDTO));
        assertEquals(StatusCode.SUCCESS,userService.loginUser(childLoginDto));
